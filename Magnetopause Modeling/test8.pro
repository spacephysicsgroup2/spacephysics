...
for i=0,n_l-1 do begin
  ...
  oplot,[14.,14.],[26.,26.],psym=8,symsize=0.4,color=fsc_color('red')
  
  xyouts,15.,26.,'Equllibrium Point'
  
  oplot,[9.,14.],[24.,24.],thick=2,color=fsc_color('red')
  ...
  K_coeff=(0.881*5.*Mach^2)/((5.*Mach^2)-1.)-Np[i]*(1e6)*(1.38e-23)*(1e-9)*T[i]/Dp[i]
  ...
  oplot,x_pause,z_pause,symsize=0.4,psym=8,color=fsc_color('red')
  ...
  oplot,x_magnetopause,z_magnetopause,thick=2,color=fsc_color('red')
  ...
  filename='empirical_'+string(year[i],format='(I4.4)')+string(month[i],format='(I2.2)')+string(day[i],format='(I2.2)')+string(hour[i],format='(I2.2)')+string(minute[i],format='(I2.2)')+string(second[i],format='(I2.2)')+'_Magnetopause.png'
  ...
  if x_magnetopause[x_concavity_index] gt 0. then write_png,'empirical'+string(i)+'.png',image1
...
write_csv,'example1_empirical.csv',time_csv_1,x_pause_csv,z_pause_csv,Bz_GSM_csv,Dp_csv

write_csv,'example2_empirical.csv',time_csv_2,x_shue_csv,y_shue_csv
